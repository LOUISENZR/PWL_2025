                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <select class="form-control" id="barang_id" name="barang_id" required>
                                ...
                                @foreach ($barang as $b)
                                    <option value="{{ $b->barang_id }}" @if ($b->barang_id == $stok->barang_id) selected @endif>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <select class="form-control" id="user_id" name="user_id" required>
                                ...
                                @foreach ($user as $u)
                                    <option value="{{ $u->user_id }}" @if ($u->user_id == $stok->user_id) selected @endif>
                                        {{ $u->nama }}</option>
                                @endforeach
                            </select>
                            @error('user_id')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <input type="datetime-local" class="form-control" id="stok_tanggal" name="stok_tanggal"
                                value="{{ old('stok_tanggal', \Carbon\Carbon::parse($stok->stok_tanggal)->format('Y-m-d\TH:i')) }}"
                                required>
                            @error('stok_tanggal')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
